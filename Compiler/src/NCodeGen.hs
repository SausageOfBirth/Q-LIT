...
customGateAssign :: String -> Integer -> Integer -> String -> 
                    [[CExpr]] -> String
customGateAssign name _ _ str [] = str
customGateAssign name col row str ([c]:ys) = 
  case c of
  Complex r i ->
    if i >= 0 then 
      customGateAssign name 0 (row+1) 
      (str++name++"_data["++show row++"]["++show col++"] = "
      ++show r++"+"++show i++" * I;\n ") ys
    else
      customGateAssign name 0 (row+1) 
      (str++name++"_data["++show row++"]["++show col++"] = "
      ++show r++show i++" * I;\n ") ys
  Constant c -> customGateAssign name 0 (row+1) (pickCon c) ys
    where pickCon c
            | c == "e"  = str++name++"_data["++show row++"]["++show col++
                   "] = M_E + 0.0 * I;\n"
            | c == "pi" = str++name++"_data["++show row++"]["++show col++
                   "] = M_PI + 0.0 * I;\n"
customGateAssign name col row str ((c:xs):ys) = 
  case c of
  Complex r i ->
    if i >= 0 then 
      customGateAssign name (col+1) row 
      (str++name++"_data["++show row++"]["++show col++"] = "
      ++show r++"+"++show i++" * I;\n ") (xs:ys)
    else
      customGateAssign name (col+1) row
      (str++name++"_data["++show row++"]["++show col++"] = "
      ++show r++show i++" * I;\n ") (xs:ys)
  Constant c -> customGateAssign name (col+1) row (pickCon c) (xs:ys)
    where pickCon c
            | c == "e"  = str++name++"_data["++show row++"]["++show col++
                   "] = M_E;\n"
            | c == "pi" = str++name++"_data["++show row++"]["++show col++
...
customGate name n = 
  (customGateAssign name 0 0 
  ("Matrix* "++name++" = matrixInit("++show (length n)++","++show 
  (length n)++");\n "++"double complex **"++name++"_data = "++name++
  "->data;\n ") n) ++name++"->data = "++name++"_data;\n "

-- generates a tensor product between wires in a slot?
makeCircSlot :: [CQGate] -> String -> String
makeCircSlot [] str = str
makeCircSlot [x] str =
  let (x',n,m,qubit,c) = x in
    if x' == "measurement"
    then qubit++" = measurement("++qubit++","++show (m!!0)++");\n "
    else if c
    then str++"slot_"++qubit++show n++" = tensor(slot_"++qubit++show n++","
         ++"controlGen("++show(head m)++","++show(last m)++","++x'++"));\n"
    else str++"slot_"++qubit++show n++" = tensor(slot_"++qubit++show n++","++x'++");\n "
         ++qubit++" = multBitGate(slot_"++qubit++show n++","++qubit++");\n "
makeCircSlot (x:xs) str = 
  let (x',n,m,qubit,c) = x in 
    if x' == "measurement" 
    then qubit++" = measurement("++qubit++","++show (m!!0)++");\n "
    else if c
    then makeCircSlot xs (str++"slot_"++qubit++show n++" = tensor(slot_"++qubit++show n++","
         ++"controlGen("++show(head m)++","++show(last m)++","++x'++"));\n")
    else makeCircSlot xs (str++"slot_"++qubit++show n++" = tensor(slot_"++qubit++show n++","
         ++x'++");\n")
...
makeCirc dim cs = 
  ...
  in (map (\(x:xs) -> 
    let (x',n,m,qubit,c) = x in 
      if xs == []
        then if c
        then "Matrix* slot_"++qubit++show n++" = controlGen("++show(head m)++","++show(last m)++","++x'++");\n "++qubit++" = multBitGate(slot_"++qubit++show n++","++qubit++");\n "
        else "Matrix* slot_"++qubit++show n++" = "++x'++";\n "++qubit++" = multBitGate(slot_"++qubit++show n++","++qubit++");\n "
      else if c
        then 
          if x' == "measurement"
            then qubit++" = measurement("++qubit++","++show (m!!0)++");\n "
            else makeCircSlot xs ("Matrix* slot_"++qubit++show n++" = controlGen("++show(head m)++","++show(last m)++","++x'++");\n ")
        else 
          if x' == "measurement"
            then qubit++" = measurement("++qubit++","++show (m!!0)++");\n "
            else makeCircSlot xs ("Matrix* slot_"++qubit++show n++" = "++x'++";\n ")) slots)++["outputQ(\"qubitoutput_"++circ++"\","++circ++");\n"]
...
qubitDec [] = ""
qubitDec ((name,Bra content):xs) = 
  "Qubit* "++name++" = "++qubitProd content++";\n "++(qubitDec xs)
qubitDec ((name,Ket content):xs) = 
  "Qubit* "++name++" = "++qubitProd content++";\n "++(qubitDec xs)
